...
io.on("connection", (socket) => {
    ...
    if (index !== -1) {
      ...
      if (socket.esMiTurno) {
        const siguienteTurno =
          usuariosJuego.length > 0 ? usuariosJuego[0].id : null;
  ...
  socket.on("respuestaJugador", ({ userName, paisId, acertado }) => {
    ...
    io.emit("comprobarColorActualMapa", {
      idPais: paisId,
      color: color,
      acertado: acertado,
      color0: usuariosJuego[0].color,
      color1: usuariosJuego[1].color,
    });
    ...
    if (conquistasJugador1 === totalPaises) {
      io.emit("partidaFinalizada", { ganador: usuariosJuego[0].nombreUsuario });
      io.emit("cambiarAPartidaFinalizada", {
        usuarioGanador: usuariosJuego[0].nombreUsuario,
      });
    } else if (conquistasJugador2 === totalPaises) {
      io.emit("partidaFinalizada", { ganador: usuariosJuego[1].nombreUsuario });
      io.emit("cambiarAPartidaFinalizada", {
        usuarioGanador: usuariosJuego[1].nombreUsuario,
      });
    }
  });
});
